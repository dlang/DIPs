...
This works well in statement based code, but D also supports more expression based coding styles where this flow analysis info is lost because functions act as a 'boundrary'.
For example: ([courtesy to Paul Backus](https://forum.dlang.org/post/hdheyjpxiqanakwpisbf@forum.dlang.org)).
```D
struct Expected(T, E) {
...
A bottom type can also be used in a ternary operator: (example [courtesy to Andrei Alexandrescu](https://forum.dlang.org/post/ok5bcj$5d8$1@digitalmars.com))
```D
...
When asking the reference compiler what `typeof(*null)` is, it tells `Error: can only * a pointer, not a typeof(null)`.
...
Unlike `size_t` where just `size` could clash with a variable name, it is not deemed necessary for `never` or `noreturn` to have a `_t` postfix.
...
```
This can be changed of course, but adding the alias `noreturn` wouldn't even break this since overriding an alias is allowed (you can name a variable `string` too if you want).

While the name `TBottom` was proposed in DIP 1017, it is highly disagreed upon.
...
While on the stable channel it still acts as a special type only found as a return type of functions, making it a full-fledged type is being worked on: see [Rust issue 35121](https://github.com/rust-lang/rust/issues/35121).
...
After this list, some aspects of the new behavior will be elaborated.
...
**(2) All built-in operators that have "overloads" act like they have specialized versions for `noreturn`.**

E.g. the expression `assert(0) + assert(0)` would normally get an ambiguity error: is it `int`, `long`, `float` or `double` addition?
...
In practice it doesn't matter of course, in any case the resulting expression has type `int[]` and compiles to:
```D
auto __tmp = [1, 2, 3];
assert(0);
```
...
- 3 and 4 ensure you can use the function pointer to `noreturn exit()` and pass it to a `int function() callback` parameter without explicit cast (which disables *all* checking for matching parameter types).
- 5, 6 and 7 mean that you can still assign `null` (which is of type `noreturn*`) to a function pointer or array.

Note that rules 1 to 4 don't naturally follow from rule 0 since pointers and arrays are currently not covariant in element types, and function pointers are not covariant in return types.
This means that I can define `class C : Object`, but that doesn't mean `is(C[] : Object[])`.
...
The `throw` keyword acts as a unary operator with the same low operator prescedence as `cast()`.
In practice most throw expressions will just look like `throw new Exception()`, but there is a chance that disambiguation is needed when operator overloading is used:
```D
...
Since D doesn't allow skipping over initializations with e.g. `goto`, this simply means at the point of declaration.

Using void-initialization does not make a difference on a `noreturn` field:
It can be seen as giving the type a random value with a biased distribution (based on garbage memory on the stack).
...
If it turns out that inheriting from `noreturn` makes sense in generic code, this restriction can always be lifted in the future.
...
The addition of a bottom type doesn't change these rules: an override of a function returning a `T` may have return type `noreturn` (since `noreturn` is a subtype of every `T`) but a parameter of type `noreturn` may not be replaced by `T` (despite `T` being a supertype of `noreturn`).

### Grammar changes
Throw stratements become expressions so they can be used in lambda functions.
...
### Limitations
While C functions can be declared using the noreturn type because of the implicit conversion and no name mangling, C++ function pointers have the return type in their mangling.
