...
Instead, *every* function that touches `ptr` and `length` must be manually checked.
...
Even when the `int` is wrapped in a `struct`, no `scope` checking is being done:
```D
struct File
{
    private int fd;
...
While reading a `@system` variable could be allowed when it has a safe type, it was chosen to restrict this as well to make the rules less complex.
...
```
However, currently Contract Programming is a separate feature from Memory Safety, and an empty `invariant {}` block looks like something that can be safely removed.
