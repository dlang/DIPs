...
* The description says that `scope` "is not stripped away from an aggregate with at least one @system field, even when the aggregate has no members that contain pointers." The example noted in the item above appears to be the only justification, so this sentence should be removed. Se the DIP author's response to the previous item.
