...
- If `x` is a function, variable or aggregate field, return value indicates whether it is marked `@safe`, `@trusted` or inferred `@safe`.
- If `x` is a type, return value indicates whether it is a safe type, as described in [the background section](#background).
