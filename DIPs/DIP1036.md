...
the parameters. The user just can use an efficient literal that shows the data
exactly as he/she wants it to be, and the receiving function can interpret the
parameters as if they wrote the parameters out individually.

This DIP proposes to cover both cases with a single construct.
...
`InterpolatedString`, which starts with an `i` prefix, and then is followed by
a specialized string literal. The string literal portion can be equivalent to one of
`WysiwygString`, `AlternateWysiwygString`, `DoubleQuotedString`, or
`TokenString`. However, inside those constructs, you can have an interpolation
sequence. An interpolation sequence consists of:
...
The `Tokens` can consist of any D expression that can properly parse as an `Expression` (including nested interpolated strings).

The `InterpolationSequence` can be escaped only in a `DoubleQuotedString` literal by prepending a backslash to the `$` that begins the sequence. Example:
...
This DIP does not specify the requirements for escaping inside an
`InterpolationSequence` for string literals. It is our belief that a reasonable
escape system can be achieved that satisfies both the lexer and parser. If
questions arise surrounding this part of the DIP, we will address those in further
revisions.

The Grammar changes required for this are TBD, but should be based on existing string literal grammar. The DIP authors are not experts in grammar or the front end implementation, and we implore the community and the language mantainers to find a suitable way to specify this such that it fits the current implementation and is feasible. While we are not providing a grammar, the fact that `TokenString` provides much of the same features leads us to believe that a solution does exist. In the event this becomes a sticking point, we are willing to limit the interpolation string syntax.
...
The compiler will lex and parse the string literal into a sequence of expressions. The expressions will consist of `InterpolationLiteral` expressions and interpolated expressions from the `InterpolationSequence` items. No two `InterpolationSequence` expressions will be consecutive, an empty `InterpolationLiteral` will be inserted to separate them. The sequence will also be identified internally by the compiler so the original `InterpolationString` can be easily rewritten if necessary (see below).
...
3. If the type is a struct, class or interface, and the type defines a method or field member `toString` which yields a string, or can be used to construct a string via a provided delegate, that is used.
4. If the type is a struct without a suitable toString method, it is converted to its fully qualified type name, followed by '(', and then member is translated recursively, followed by a trailing ')'.
5. If the type is a class or interface without a suitable toString method, the fully qualified name of the class/interface is used followed by '<', the hex address of the interface/object, and then '>'.

In all cases, the exact definition of the translation should be in the language spec (so users can rely on the resulting transformation not changing). The complete specification of these translations is left up to the eventual implementors and language maintainers. Any variation of output style/locale must be handled by user-defined functions that accept the sequence directly (see next section).
...
Concatenation of 2 interpolation strings using the `~` operator will result in `idup` being called on both literals, and the result concatenated together. This DIP allows for (and reccommends) the compiler to shortcut this and pass both sequences to the same `idup` call. However, this may be considered an optimization that is not necesssary for the feature to exist.

To pass 2 sequential interpolation strings to a function that accepts interpolation strings, concatenation is not needed, and just separating the string literals by a comma will suffice. This allows a single interpolation string to be split into multiple lines. One may also use string literal types that allow multi-line strings.
...
Properly formatting floating point values with phobos/druntime currently requires libc to work. This DIP does not address this external dependency, but it is something to consider for a language feature.
