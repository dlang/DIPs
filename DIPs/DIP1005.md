...
We propose an addition to the D language that allows the use of the keyword `import` as part of any declaration. When that syntax is used, it instructs the compiler to execut the import before looking up any names in the declaration. To clarify by means of examples, the previous examples can be rewritten as:
