...
into difficulty when resources (including but not limited to memory) need to be
reclaimed early.
...
`scope` provides a mechanism to guarantee that a reference cannot escape lexical
scope.
...
For each value `v` within a program we define the notion of _reachability_
denoted as `reachability(v)`, akin to the lexical extent through which the value
can be accessed.

- For an unnamed temporary (either lvalue such as `array[f()]` or rvalue such as
`42 + f()`), the reachability is the largest grammatical expression within which
it is used.
- For a named variable in a scope, the reachability is the lexical scope of the variable per the language rules.
- For a module-level variable, reachability is considered infinite. Notation:
`reachability(v)` `=` `∞`.

For values with finite reachability, we denote their reachability as a tuple
`M:B:E`, where:

- `M` is a string that uniquely identifies the module within the program
- `B` is an integral that is the index of the character at which reachability
begins, as follows:
  - For temporaries, `B` is the index of the first character of the largest
expression in source code, and `E` is the index of the last character of the
same largest expression
  - For named values, `B` is the index of the `;` or `,` that ends the
definition of the value, and `E` is the index of the `}` that closes the scope
of the definition.
  - For `ref` and `out` parameters, `B` is the index of the opening brace `{`
of the function body, and `E` is the closing brace `}` of the function body.

(This definition clarifies that e.g. a defined variable is not reachable during
its own definition, e.g. `int x = x;` does not initialize `x` from itself, but
only possibly from a global with the same name. It also clarifies that e.g. a
function declaration of the form `void f(int a, int b = a)` cannot use the
first parameter as a default value for the second.)

Due to language scoping rules, reachabilities cannot partially intersect or
"cross": for any two values, either they are not simultaneously reachable at
all, or one's reachability is included within the other's. (In particular,
reachability is identical for all unnamed temporaries within a given expression
and also for all module-level values.) We define a partial order  among
reachabilities: `reachability(v1) <= reachability(v2)` if `v2` is reachable
through all portions of program when `v1` is reachable (including the case where
both values have the same reachability). If two variables have disjoint
reachabilities, they are unordered. Mathematically, if `reachability(v1) =
M1:B1:E1` and `reachability(v2) = M2:B2:E2`, then `reachability(v1) <=
reachability(v2)` if and only if `M1 == M2`, `B1 >= B2`, and `E1 <= E2`. If
either inequality is strict, we `reachability(v1) < reachability(v2)` is also
true.

##### Lifetime

We also define the notion of _lifetime_ for each value `lifetime(v)`, which is
the extent during which a value can be safely used.

- For values allocated on the garbage collected heap, lifetime is infinite
whilst reachability is dependent on the references in the program bound to those
values. We write `lifetime(v) = ∞`.
- For all global and `static` variables, lifetime is again infinite
(`lifetime(v) = ∞`).
- For types without indirections such as `int`, `lifetime(v) = reachability(v)`.
- For `ref` function parameters, reachability is within the scope of the
function, whereas lifetime is that of the expression used to invoke the
function.
- For an unrestricted pointer, reachability is defined as above. Lifetime,
however, is dictated by the lifetime of the data to which the pointer points to.
Lifetime may be ended dynamically by the program deallocating the memory to
which the pointer is referring.
...
dangerous, albeit not necessarily incorrect state. The behavior becomes
