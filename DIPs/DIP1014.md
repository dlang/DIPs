...
The point was made that allowing `opPostMove` to be overidden raises the question of what to do when it is annotated with `@disable`. The DIP author ultimately suggested it should be handled the same way an `opAssign` is currently handled for classes. 
