...
performance are almost certainly prohibitive. Intra-procedural dataflow
analysis, on the other hand, is more feasible to implement, but would still
allow false negatives in some cases. For example:
...
This friction will be particularly severe for programmers attempting to make
...
likely result is a language feature that, while theoretically more sound, would
provide less utility to D programmers in practice than the design proposed by
this DIP.
...
to a type. The resulting design would be strictly less complex in terms of both
specification and implementation than the one this DIP proposes, and would not
introduce any risk of false positives.

However, adopting this design would introduce friction of a different sort: a
greater barrier to adoption. Consider the following example:
...
prospect of doing this might discourage us from using `@nodiscard` in the first
place.

Adopting `@nodiscard` as an attribute only for types, without the syntax-level
check, would leave D programmers with only choice (2). As a result, while the
programmers who did choose to use `@nodiscard` might be better off, there would
be fewer of them than there would be if choice (1) were also available.

On balance, the author belives that the design proposed by this DIP is likely
to be more useful to D programmers in practice than one which excludes the
syntax-level check. However, adopting `@nodiscard` only as a type attribute
with a type-level check would be a reasonable compromise if the syntax-level
check is deemed inadequate.
