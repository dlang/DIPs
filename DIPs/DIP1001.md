...
@throws(auto) infers all thrown types

functions that return auto or function templates and don't have a explict
@throws() have @throws(auto).
...
A thrown type must be implictly convertable to one of the types mentioned in
the @throws(TypeList) list.

This fits in with the old class based exception handling.

A possible optimization would be to generate a seperate unwinding table for
each value type.
...
Main Reason to include this is so Throw Any Type and Error Code Unwinding
...
Due to C++'s header files and legacy ploblem, exceptions had to be checked at runtime,
This is not the case with This proposal's @throws attribute, they can be checked at
compile time.

Unlike C++'s throws lists, @throws is usable in templates because they will be applied
automatically.

@throws being a attribute, may be applied to multiple functions at once.
...
Implict Conversion:
	...
	void throwLong() @throws(long)
	{
		//implictly converts to long
		throw 0;
	}
## Copyright & License
