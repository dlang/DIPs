...
error of the following nature: "Non of the forward declarations of function
...
Compile can go one step further, and guess the intended forward declaration the
...
Named arguments proposed by this DIP does not permitted skipping default parameters.

#### Variadic functions

The variadic part of the arguments can be labeled with the name of the
variadic parameter and a subscript.
...
Uniform function calls can contain argument names just like normal function
calls.
...
If a function is defined with some of its parameter names omitted, argument
to those parameters can labeled with an empty name.
...
Overridden member functions can be called with set of names matching any of its
definitions visible through the object used to call those functions.
...
**Allow calling same function with different set of names:** This is achieved by
adding a forward declarations with a different set of parameter names. Caller can use names matching
any of the forward declarations. For
example:
...
Also, forward declarations can be annotated with `deprecated` to indicate that a
certain set of parameter names are deprecated.

### Overloading and name mangling

Named arguments has no impact on function overloading. Argument name check is
only performed _after_ function overload is resolved.
...
The compiler should generate more useful error message in this case, it should at
least inform the user that named arguments play no role in function overloading.

Named arguments also don't participate in name mangling.

## Alternatives

There are several alternative ways to implement named argument, or something
similar to it. However, they generally add noise to function calls and/or
require fundamental changes to how functions are defined.
...
Also, this solution limits the name of variables you can use as arguments.
...
omission of default argument by using named argument,
