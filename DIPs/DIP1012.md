...
During the deprecation phase the compiler shall treat `pure` and `nothrow` (without a leading `@`) as though they did have a leading `@`.
...
template Implements(Model,alias _Idol, Args...)
{
   ...
   enum bool Implements = staticFold!(staticMap!(signaturesMatch, __traits(getMembers,Idol)));
}
