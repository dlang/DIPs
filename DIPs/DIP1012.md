...
* `inferred`. Indicates the compiler shall determine the value of the attribute. It is illegal for a function declaration without a body to be marked `inferred`. Does not apply where the compiler is not responsible for inference e.g. `final`/`virtual`.
...
to create a custom runtime,e.g. final by default for embedded systems.
...
the attribute category's default value (e.g. gc, impure, system, thows)
