...
### `@property` and implicit wideing casts

`@property` and implicit wideing casts are syntactically identical to public feilds and variables respectively and so a change in the API
from one to the other would result in silent change to the users code. 
For this reason, they promote to`const ref` rather than `ref`, since this guaruntees that the caller is not expecting the value to change as a result of the call. 
All other cases (i.e. not `@property` or implicit wideing casts) the expressions that give rise to the promoted rvalue-to-ref do not outlive the scope of the call expression so there is no risk of such use after expected modification.

e.g.
```d
//API v1
void foo(ref short i)
{
    i = 42;
}
//API v2
...
Under the change from v1 to v2 of the API the users code would otherwise continue to compile but produce unexpected results. But if the implicit promotion is to `const ref` then the code fails to compile and the user is forced to acknowledge the change in behaviour.
